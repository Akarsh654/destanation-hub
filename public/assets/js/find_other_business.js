(() => {
    ...
    var initTable = function () {
        ...
        table.DataTable({
            responsive: true,
            ajax: {
                url:`/find-other-businesses/${user_id}`,
				type: 'GET',
				headers: {
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, full, meta) {
                        return `\
                            <a id = "${data.businessWebsiteUrl}"  href="javascript:;" class="claim_btn btn btn-primary">\
                                    <i class="la flaticon-plus"></i> Connect\
                            </a>\
                            <a  id="${data._id}" data-business-type = "${data.businessType}" data-business-address = "${data.businessAddress}" data-business-mobile-no - "${data.businessMobileNumber}" data-business-description = "${data.businessDescription}" data-business-websiteurl = "${data.businessWebsiteUrl}" data-business-name="${data.businessName}" data-toggle= "modal" data-target="#playerModal" href="javascript:;" class="view_btn btn btn-success">\
                                <i class="la flaticon-eye"></i> View\
                            </a>\
    ...
    $(document).ready(() => {
        ...
        $('#kt_datatable').on('click', '.view_btn',(event)=>{
            let id = event.target.id
            let playerModal = $('#playerModal');
            // alert($('#'+id).attr('id').val())
            let business_name = $(`#${id}`).attr('data-business-name')
            let business_url = $(`#${id}`).attr('data-business-websiteurl')
            let business_description = $(`#${id}`).attr('data-business-description')
            let business_address = $(`#${id}`).attr('data-business-address')
            let business_type = $(`#${id}`).attr('data-business-type')
            playerModal.find("#businessName").val(business_name);
            // playerModal.find("#business_mobile_no").val($(`#${id}`).attr('data-business-mobile-no'));
            playerModal.find("#business_website_url").val(business_url);
            playerModal.find("#business_description").val(business_description); 
			playerModal.find("#business_address").val(business_address); 
			playerModal.find('#business_type').val(business_type)
        ...
        $('#kt_datatable').on('click', '.claim_btn',(event)=>{
            ...
            swal.fire({
                text: "A connection request has been successfully sent to the Business Owner",
                icon: "success",
